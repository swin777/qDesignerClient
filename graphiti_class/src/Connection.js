 ...
 *     //
 *     c.setSourceDecorator(new graphiti.decoration.connection.BarDecorator());
 ...
 * {@link graphiti.shape.basic.PolyLine PolyLine}.<br>
 * The source and target endpoints of a connection are each defined using a {@link graphiti.layout.anchor.ConnectionAnchor ConnectionAnchor}. 
 * These endpoints, along with any other points on the connection, are set by the connection's  {@link graphiti.layout.connection.ConnectionRouter ConnectionRouter}. 
 * <br>
 ...
 * The ends are placed with the help of {@link graphiti.layout.anchor.ConnectionAnchor anchors}. An 
 * {@link graphiti.layout.anchor.ConnectionAnchor anchors} is a fixed or calculated location, usually associated with some 
 * figure. For example, the {@link graphiti.layout.anchor.ChopboxConnectionAnchor ChopboxAnchor} finds the point at which a 
 ...
 * connection by calling {@link graphiti.shape.basic.PolyLine#addPoint Polyline.addPoint}. The connection's existing point list 
 ...
 * decorate the connection. The placement of each type of decoration can vary, so a {@link graphiti.layout.locator.ConnectionLocator ConnectionLocator} 
 * is used to delegate to each child's constraint object, a Locator. <br>
 * <br>
 * {@link graphiti.decoration.connection.Decorator Decorator} can be used to create and render a rotatable shape at 
 * the end or start of a connection like arrows or boxes. Examples are {@link graphiti.decoration.connection.ArrowDecorator ArrowDecorator},  
 * {@link graphiti.decoration.connection.BarDecorator BarDecorator} or {@link graphiti.decoration.connection.CircleDecorator CircleDecorator}
 * <br>
 ...
 * @inheritable
 * @author Andreas Herz
 * @extends graphiti.shape.basic.PolyLine
 */
...
//    DEFAULT_ROUTER: new graphiti.layout.connection.BezierConnectionRouter(),
    ...
    **/
     ...
     * 
     * @param {graphiti.layout.connection.ConnectionRouter} [router] the new router for this connection or null if the connection should use the default routing
     **/
    ...
    setPersistentAttributes : function(memento)
    {
          ...
          if(typeof DEFAULT_ROUTER == "undefined"){
	  	  	this.router = new graphiti.layout.connection.DirectRouter();
	  	  }
	  	  ...
	  	  if(typeof this.connectType != "undefined" && this.connectType=="Direct"){
	  	  	...
	  	  	this.router = new graphiti.layout.connection.ManhattanBridgedConnectionRouter();
	  	  }else  if(typeof this.connectType != "undefined" && this.connectType=="Bezier"){
	  	  	this.router = new graphiti.layout.connection.BezierConnectionRouter();
	  	  }
 ...
 * 
 * @param {graphiti.port} sourcePort port of the source of the connection
 * @param {graphiti.port} targetPort port of the target of the connection
 * @template
 * @returns {graphiti.Connection}
 */
graphiti.Connection.createConnection=function(sourcePort, targetPort){
    
    return new graphiti.Connection();
};
