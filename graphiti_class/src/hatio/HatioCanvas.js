/**
 * @class hatio.HatioCanvas
 * 
 * See the example:
 *
 *     var canvas =  new hatio.HatioCanvas(canvasDivId);
 *     
 * @extends graphiti.Canvas
 */
hatio.HatioCanvas = graphiti.Canvas.extend({
	
	NAME : "hatio.HatioCanvas",
	...
	init:function(id){
		...
		this.currentMultiSelection = new graphiti.util.ArrayList();
		this.resizeHandleArr = new graphiti.util.ArrayList(); 
    ...
    onMouseDown : function(x, y, clickFigure){
		...
		if (typeof clickFigure != "undefined" && clickFigure!==null){
			...
			if(figure instanceof hatio.shape.node.basic.Group){
				figure = null;
			}
			if(figure!=null && !(figure instanceof graphiti.shape.basic.Line)){
				return;
			}
		}
        if(figure===null){
        ...
        if(this.currentMultiSelection.getSize()>0 && figure!==null && !(figure instanceof graphiti.ResizeHandle)){ //여러개 선택인지 검사
    	...
    	}else if(this.currentMultiSelection.getSize()>0 && figure!==null && (figure instanceof graphiti.ResizeHandle)){
    	...
    	//figure instanceof graphiti.ResizeHandle
        ...
        if(figure instanceof graphiti.Port && !(figure instanceof hatio.VolatilePort)){
        ...
        if (figure !== this.currentSelection && figure !== null && figure.isSelectable() === true) {
            ...
            this.selectRectangle = new graphiti.shape.basic.Rectangle();
    ...
    figureDrag : function(/* :int */dx,/* :int */dy, mouseDragFigure){
    	// it is only neccessary to repaint all connections if we change the layout of any connection
            ...
            if(this.linesToRepaintAfterDragDrop.isEmpty()===true && (mouseDragFigure instanceof graphiti.shape.node.Node)){
	        ...
	        for( i=0; i< figures.getSize();i++){
	        	...
	        	if(figure!==rectangle){
	            	...
	            	if(fiSX > xMin && fiSY > yMin && fiEX < xMax && fiEY < yMax){
		        		...
		        		figure.resizeHandle1 = local_resizeHandle1 = new graphiti.ResizeHandle(this,1); // 1 = LEFT TOP
				        figure.resizeHandle2 = local_resizeHandle2 = new graphiti.ResizeHandle(this,2); // 2 = CENTER_TOP
				        figure.resizeHandle3 = local_resizeHandle3 = new graphiti.ResizeHandle(this,3); // 3 = RIGHT_TOP
				        figure.resizeHandle4 = local_resizeHandle4 = new graphiti.ResizeHandle(this,4); // 4 = RIGHT_MIDDLE
				        figure.resizeHandle5 = local_resizeHandle5 = new graphiti.ResizeHandle(this,5); // 5 = RIGHT_BOTTOM
				        figure.resizeHandle6 = local_resizeHandle6 = new graphiti.ResizeHandle(this,6); // 6 = CENTER_BOTTOM
				        figure.resizeHandle7 = local_resizeHandle7 = new graphiti.ResizeHandle(this,7); // 7 = LEFT_BOTTOM
				        figure.resizeHandle8 = local_resizeHandle8 = new graphiti.ResizeHandle(this,8); // 8 = LEFT_MIDDLE
				        figure.volatilePort =  local_volatilePort = new hatio.VolatilePort(this);
    ...
    removeFigureCommand:function(){
    	...
    	if(node!==null){
    		var command = new graphiti.command.CommandDelete(node);
			this.getCommandStack().execute(command);	
    	}
    	
        for(i=0; i<this.currentMultiSelection.getSize(); i++){
        	var command2 = new graphiti.command.CommandDelete(this.currentMultiSelection.get(i));
    ...
    getBestFigure : function(x, y, figureToIgnore)
    {
        ...
        // MultiSeiect ResizeHandles first
    ...
    {
        var figure;
        if(type=='hatio.shape.node.basic.CustomSvgFigure'){
        	figure = new hatio.shape.node.basic.CustomSvgFigure(originalFigure.node_id);
    ...
    autoLayOut : function(algorithmType){
        var nodes = new graphiti.util.ArrayList();
        ...
        var connects = new graphiti.util.ArrayList();
    	...
    	var figures = this.getFigures();
		$.ajax({
		    url: 'http://hatio-graph-layout.appspot.com/service.jsp',
                 ...
                 for(var i=0; i<objArr.length; i++){
                 	for(var j=0; j<figures.getSize(); j++){
						...
						if(fig.id==objArr[i].id){
							var command = new graphiti.command.CommandMove(fig);
		...
		// var delCommArr = new graphiti.util.ArrayList();
	  	// for(var s=0; s<this.getLines().getSize(); s++){
    		// var conn = this.getLines().get(s);
    		// var command = new graphiti.command.CommandDelete(conn);
...
//     	
        ...
        for(var s=0; s<this.getLines().getSize(); s++){
    		var conn = this.getLines().get(s);
    		conn.setRouter(new graphiti.layout.connection.DirectRouter());
    ...
    minimumDistanceConnect : function(conn){
		...
		conn.setTargetDecorator(new graphiti.decoration.connection.ArrowDecorator());
   	 	...
   	 	var command = new graphiti.command.CommandAdd(this, conn);
   	 	command.execute();
    },
    ...
    // minimumDistanceConnect : function(sourceFigure, targetFigure){
    	// var sourcePortArr = sourceFigure.getPorts();
      	...
      	// }
//       	
      	// var conn = new graphiti.Connection("DirectRouter");
		// conn.setTargetDecorator(new graphiti.decoration.connection.ArrowDecorator());
   	 	// conn.setSource(selectPortInfo.startPort);
   	 	// conn.setTarget(selectPortInfo.endPort);
   	 	// var command = new graphiti.command.CommandAdd(this, conn);
    ...
    labelViewToogle : function(){
		...
		for ( var i = 0; i < this.figures.getSize(); i++){
			...
			if(children.getSize()>0){
				...
				var label = new graphiti.shape.basic.Label("none");
		        //label.setColor("#000000");
		      	label.setFontColor("#0000ff");
		      	label.setStroke(0);
		      	label.setVisible(!this.labelView);
		        figure.addFigure(label, new graphiti.layout.locator.BottomLocator(figure));
			}
		}
   },
